...
// The numbers behind the weekly summary: what the AI's sentences are built from, so they can be
// judged. Pure, from the week's raw events. Hours are LOCAL hours of the day pooled over the week.
// Returns { hasData, lines: [{label, value, note?}] }.
export function buildWeekDrivers({ events }) {
  let tracked = 0, slouch = 0, breaks = 0, longestSit = 0;
  const days = new Set(), byType = {}, hourTracked = {}, hourSlouch = {};
  events.forEach((e) => {
    const dur = e.duration_seconds || 0;
    if (e.type === 'presence') {
      tracked += dur;
      longestSit = Math.max(longestSit, dur);
      if (dur >= 60 && e.date) days.add(e.date);
      if (e.start_time) addByHour(hourTracked, e.start_time, dur);
    } else if (SLOUCH_LABELS[e.type]) {
      slouch += dur;
      byType[e.type] = (byType[e.type] || 0) + dur;
  ...
  if (tracked < 3600) return { hasData: false };

  const pct = (n, d) => Math.round((Math.min(n, d) / d) * 100);
  const lines = [];
  lines.push({ label: 'tracked', value: `${formatDuration(tracked)} over ${days.size} day${days.size === 1 ? '' : 's'}` });

  const types = Object.entries(byType).sort((a, b) => b[1] - a[1]);
  const parts = types.slice(0, 3).filter(([, s]) => pct(s, tracked) >= 1).map(([t, s]) => `${SLOUCH_LABELS[t]} ${pct(s, tracked)}%`);
  const note = types[0] && types[0][0] === 'sitting_low' && slouch > 0 && types[0][1] / slouch >= 0.4
    ? 'sitting low can read high after a break, so treat the hours below with care' : null;
  lines.push({ label: 'out of plumb', value: `${pct(slouch, tracked)}%${parts.length ? `: ${parts.join(', ')}` : ''}`, note });

  const hours = Object.keys(hourTracked).map(Number).filter((h) => hourTracked[h] >= 60 * 60)
    .map((h) => ({ h, ratio: Math.min(1, (hourSlouch[h] || 0) / hourTracked[h]) }));
  if (hours.length >= 2) {
    const rough = [...hours].sort((a, b) => b.ratio - a.ratio)[0];
    const steady = [...hours].sort((a, b) => a.ratio - b.ratio)[0];
    if (rough.h !== steady.h && rough.ratio >= 0.2) {
      lines.push({ label: 'hours', value: `roughest ${hourLabel(rough.h)} (${Math.round(rough.ratio * 100)}% out of plumb), steadiest ${hourLabel(steady.h)} (${Math.round(steady.ratio * 100)}%)` });
    }
  }
  if (days.size > 0) lines.push({ label: 'breaks', value: `${(breaks / days.size).toFixed(1)} a day`, note: longestSit >= 90 * 60 ? `longest sit ${formatDuration(longestSit)}` : null });
