...
async function openDayWrap() {
  ...
  renderDayWrapInto(dayWrapBody, wrap);
}

// Shared by the wrap window and the "today" tab of the summary panel. `only` limits the rows shown.
function renderDayWrapInto(el, wrap, { only = null, showGoal = true } = {}) {
  el.textContent = '';
  const head = document.createElement('p');
  head.className = 'wrap-headline';
  head.textContent = wrap.headline;
  el.appendChild(head);
  wrap.rows.filter((r) => !only || only.includes(r.label)).forEach((r) => {
    const row = document.createElement('div');
...
const SLUMP_PHRASES = ["Neck's dropping — sit taller.", "Neck sinking — lengthen spine.", "Shoulders dropping — open up.", "Reset your position.", "Sinking a bit — lift through the chest.", "Spine's rounding — sit a touch taller.", "Shoulders back and down — reset."];
...
const MORNING_PHRASES = ["Good morning! A new day of tracking has started. Lets calibrate", "Morning! Ready when you are — let's make it a good day. Calibrate", "Good morning. Fresh day, fresh start — let's calibrate", "Hello, and welcome to a brand new day. Lets calibrate", "Morning! Sit tall, and let's begin calibrating."];
...
document.getElementById('activityToggle').addEventListener('click', () => {
  ...
  document.getElementById('activityToggle').setAttribute('aria-expanded', String(!alertFeed.hidden));
